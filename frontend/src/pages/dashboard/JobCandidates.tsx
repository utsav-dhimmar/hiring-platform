...
export default function JobCandidates() {
    ...
    // polling 
    useEffect(() => {
        const isAnyProcessing = candidates.some(c => c.processing_status === "processing" || !c.is_parsed);
    ...
    const columns: ColumnDef<ResumeScreeningResult>[] = useMemo(() => [
            ...
            cell: ({ row }) => (
                <div className="flex flex-col">
                    <span className="font-bold text-base text-foreground">
                        {`${row.original.first_name || ""} ${row.original.last_name || ""}`.trim() || "Unknown Candidate"}
                    </span>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const isProcessing = row.original.processing_status === "processing" || !row.original.is_parsed;
                    ...
                    return (
                        <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-muted-foreground italic">Analyzing...</span>
                            </div>
                            <Badge
                                variant="outline"
                ...
                return (
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            onClick={() => linkedin_url && window.open(linkedin_url.startsWith('http') ? linkedin_url : `https://${linkedin_url}`, '_blank')}
                        >
                        ...
                        <Button
                            ...
                            className="h-8 w-8 rounded-full hover:bg-blue-500/10 hover:text-blue-500 border-muted-foreground/20 disabled:opacity-30"
                            disabled={!github_url}
                            onClick={() => github_url && window.open(github_url.startsWith('http') ? github_url : `https://${github_url}`, '_blank')}
                        >
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center p-4 border rounded-3xl bg-card/40 backdrop-blur-md shadow-sm border-muted-foreground/10">
                    <div className="flex flex-col gap-1">
                        <h1 className="text-3xl font-extrabold tracking-tight text-foreground">{job?.title || "Loading..."}</h1>
                        <DashboardBreadcrumbs />
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm mt-1">
                            <span className="text-blue-500 font-semibold">{job?.department_name || "Department"}</span>
                            <div className="flex items-center gap-2">
                                <span className="text-muted-foreground">Status:</span>
                                {job && (
                                    <Badge variant={job.is_active ? "default" : "outline"} className="rounded-full px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider">
            ...
            {!loading && candidates?.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="group p-6 rounded-[2.5rem] border bg-card/30 backdrop-blur-sm shadow-sm flex flex-col items-center gap-2 hover:bg-card/50 transition-all duration-300 border-muted-foreground/10">
                        ...
                        <span className="text-sm font-bold text-muted-foreground uppercase tracking-widest group-hover:text-foreground transition-colors">Resumes</span>
                    </div>
                    <div className="group p-8 rounded-[2.5rem] border bg-card/30 backdrop-blur-sm shadow-sm flex flex-col items-center gap-2 hover:bg-green-500/5 transition-all duration-300 border-muted-foreground/10 hover:border-green-500/20">
                        <span className="text-4xl font-black text-green-600">{candidates.filter(c => c.pass_fail).length}</span>
                        <span className="text-sm font-bold text-green-600/70 uppercase tracking-widest group-hover:text-green-600 transition-colors">Passed</span>
                    </div>
                    <div className="group p-8 rounded-[2.5rem] border bg-card/30 backdrop-blur-sm shadow-sm flex flex-col items-center gap-2 hover:bg-red-500/5 transition-all duration-300 border-muted-foreground/10 hover:border-red-500/20">
                        <span className="text-4xl font-black text-red-500/80">{candidates.filter(c => !c.pass_fail).length}</span>
                        <span className="text-sm font-bold text-red-500/50 uppercase tracking-widest group-hover:text-red-500/80 transition-colors">Failed</span>
                    </div>
                </div>
            )}
