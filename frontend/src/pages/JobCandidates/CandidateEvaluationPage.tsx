/**
 * Candidate Evaluation Page.
 * Manages all interview stages for a specific candidate.
 * Currently supports Stage 1: HR Screening Round.
 */

import React, { useState, useEffect } from "react";
import { useParams, useNavigate, Link } from "react-router-dom";
import { Container, Breadcrumb, Row, Col, Nav, Badge } from "react-bootstrap";
import {
  PageHeader,
  Card,
  CardBody,
  ErrorDisplay,
  LoadingSpinner,
  Button,
} from "../../components/common";
import Stage1HRRound from "../../components/candidate/Stage1HRRound";
import { adminCandidateService, adminJobService } from "../../apis/admin/service";
import type { CandidateResponse } from "../../apis/types/resume";
import type { Job } from "../../apis/types/job";
import type { Stage1Info } from "../../apis/types/stage";

const CandidateEvaluationPage: React.FC = () => {
  const { jobId, candidateId } = useParams<{ jobId: string; candidateId: string }>();
  const navigate = useNavigate();

  const [candidate, setCandidate] = useState<CandidateResponse | null>(null);
  const [job, setJob] = useState<Job | null>(null);
  const [stage1, setStage1] = useState<Stage1Info | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState("stage1");

  useEffect(() => {
    const fetchData = async () => {
      if (!jobId || !candidateId) return;

      try {
        setLoading(true);
        // In a real app, these would be API calls
        // For now, we mock the data or use existing services
        const [jobData, candidateData] = await Promise.all([
          adminJobService.getJobById(jobId),
          // We don't have getCandidateById yet, so we fetch from job candidates
          adminCandidateService.getCandidatesForJob(jobId).then((candidates: CandidateResponse[]) => 
            candidates.find((c: CandidateResponse) => c.id === candidateId) || null
          )
        ]);

        if (!jobData || !candidateData) {
          setError("Job or Candidate not found");
          return;
        }

        setJob(jobData);
        setCandidate(candidateData);

        // Mock Stage 1 data for UI demonstration
        setStage1({
          id: "stage1-id",
          candidate_id: candidateId,
          job_id: jobId,
          transcript_id: null,
          status: "pending",
          analysis: null,
          hr_decision: null,
          created_at: new Date().toISOString(),
          completed_at: null,
        });

      } catch (err) {
        console.error("Failed to fetch evaluation data:", err);
        setError("Failed to load candidate evaluation data.");
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, [jobId, candidateId]);

  const handleUploadTranscript = async (file: File) => {
    console.log("Uploading transcript:", file.name);
    // Mock processing status
    setStage1(prev => prev ? { ...prev, status: "processing", transcript_id: "trans-123" } : null);
    
    // Simulate API delay
    setTimeout(() => {
      setStage1(prev => prev ? {
        ...prev,
        status: "completed",
        analysis: {
          communication_skill: 8,
          confidence: 9,
          cultural_fit: 7,
          profile_understanding: 8,
          tech_stack_alignment: 6,
          salary_alignment: 9,
          overall_score: 78,
          response_summary: "The candidate demonstrated strong communication skills and confidence. They have a good understanding of the role but may need some training on our specific tech stack.",
          communication_evaluation: "Clear and articulate. Professional tone throughout the conversation.",
          recommendation: "Proceed to Stage 2 - Technical Practical Round.",
        }
      } : null);
    }, 3000);
  };

  const handleMakeDecision = async (decision: boolean) => {
    console.log("Making decision:", decision);
    setStage1(prev => prev ? { ...prev, hr_decision: decision } : null);
  };

  if (loading) return <LoadingSpinner fullPage />;
  if (error || !candidate || !job) {
    return (
      <Container className="py-5">
        <ErrorDisplay message={error || "Data not found"} onRetry={() => navigate(`/jobs/${jobId}`)} fullPage />
      </Container>
    );
  }

  return (
    <Container className="py-5">
      <Breadcrumb className="mb-4">
        <Breadcrumb.Item linkAs={Link} linkProps={{ to: "/" }}>Jobs</Breadcrumb.Item>
        <Breadcrumb.Item linkAs={Link} linkProps={{ to: `/jobs/${jobId}` }}>{job.title}</Breadcrumb.Item>
        <Breadcrumb.Item active>Evaluation: {candidate.first_name} {candidate.last_name}</Breadcrumb.Item>
      </Breadcrumb>

      <PageHeader
        title={`${candidate.first_name} ${candidate.last_name}`}
        subtitle={`Evaluation for ${job.title}`}
        actions={
          <Button variant="outline-secondary" onClick={() => navigate(`/jobs/${jobId}`)}>
            Back to Candidates
          </Button>
        }
      />

      <Row>
        <Col lg={3}>
          <Card className="mb-4">
            <CardBody>
              <h6 className="text-muted small text-uppercase fw-bold mb-3">Interview Stages</h6>
              <Nav variant="pills" className="flex-column" activeKey={activeTab} onSelect={(k) => setActiveTab(k || "stage1")}>
                <Nav.Item>
                  <Nav.Link eventKey="stage0" disabled>
                    <div className="d-flex justify-content-between align-items-center">
                      <span>Resume Screening</span>
                      <Badge bg="success">Pass</Badge>
                    </div>
                  </Nav.Link>
                </Nav.Item>
                <Nav.Item>
                  <Nav.Link eventKey="stage1">
                    <div className="d-flex justify-content-between align-items-center">
                      <span>Stage 1: HR Round</span>
                      {stage1?.status === "completed" ? (
                        <Badge bg={stage1.hr_decision === null ? "warning" : stage1.hr_decision ? "success" : "danger"}>
                          {stage1.hr_decision === null ? "Pending Decision" : stage1.hr_decision ? "Passed" : "Rejected"}
                        </Badge>
                      ) : (
                        <Badge bg="secondary">Incomplete</Badge>
                      )}
                    </div>
                  </Nav.Link>
                </Nav.Item>
                <Nav.Item>
                  <Nav.Link eventKey="stage2" disabled>
                    <span>Stage 2: Technical Round</span>
                  </Nav.Link>
                </Nav.Item>
                <Nav.Item>
                  <Nav.Link eventKey="stage3" disabled>
                    <span>Stage 3: Panel Round</span>
                  </Nav.Link>
                </Nav.Item>
              </Nav>
            </CardBody>
          </Card>

          <Card>
            <CardBody>
              <h6 className="text-muted small text-uppercase fw-bold mb-3">Candidate Info</h6>
              <p className="mb-1"><strong>Email:</strong> {candidate.email}</p>
              <p className="mb-1"><strong>Resume Score:</strong> {candidate.resume_score?.toFixed(1)}%</p>
              <p className="mb-0"><strong>Status:</strong> {candidate.current_status || "In Process"}</p>
            </CardBody>
          </Card>
        </Col>

        <Col lg={9}>
          {activeTab === "stage1" && (
            <Stage1HRRound
              stageInfo={stage1}
              onUploadTranscript={handleUploadTranscript}
              onMakeDecision={handleMakeDecision}
            />
          )}
          
          {activeTab !== "stage1" && (
            <div className="text-center py-5 bg-light rounded border">
              <h5>Coming Soon</h5>
              <p className="text-muted">This interview stage is not yet implemented in the current phase.</p>
            </div>
          )}
        </Col>
      </Row>
    </Container>
  );
};

export default CandidateEvaluationPage;
