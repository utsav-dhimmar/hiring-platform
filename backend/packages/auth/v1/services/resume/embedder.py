...
class Embedding(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        # 'resume' | 'jd' | 'cover_letter'
    )

    entity_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        nullable=False,
        # points to resumes.id / jobs.id / cover_letters.id
    )
    ...
    truncate_dim: Mapped[int] = mapped_column(
        nullable=False,
        # 1024 — stored so you know what dim was used
    )
    ...
    __table_args__ = (
        # one embedding per entity — replace on re-embed
