...
class CandidateAdminService:
    ...
    async def get_candidates_for_job(
        ...
        xm_stmt = select(CrossJobMatch).where(CrossJobMatch.matched_job_id == job_id).options(
        ...
        for xm in cross_matches:
            ...
            threshold_val = float(xm.matched_job.passing_threshold) if xm.matched_job and xm.matched_job.passing_threshold else 65.0
            ...
            if hr_decision:
                latest_dec = next(iter(sorted(xm.candidate.hr_decisions, key=lambda d: d.decided_at, reverse=True)), None)
    ...
    async def search_candidates_for_job(
        ...
        if search_filter is not None:
            xm_stmt = xm_stmt.join(Candidate, CrossJobMatch.candidate_id == Candidate.id).where(search_filter)
    ...
    def _map_candidate_to_response(self, candidate: Candidate, target_job_id: uuid.UUID | None = None) -> CandidateResponse:
        ...
        if target_job_id:
            ...
            for d in hr_decisions:
                ...
                elif getattr(d, "job_id", None) is None and str(candidate.applied_job_id) == str(target_job_id):
                    filtered_decisions.append(d)
